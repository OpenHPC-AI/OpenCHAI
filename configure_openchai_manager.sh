...
info "Checking for OpenCHAI tar files in: ${HOSTMACHINE_REGISTRY_PATH}/${OS_VERSION}"
...
mapfile -t tar_files < <(
    find "$HOSTMACHINE_REGISTRY_PATH/$OS_VERSION" -maxdepth 1 -type f \
...
# ------------------------------------------------------------------
if (( ${#tar_files[@]} == 0 )); then
    warn "No registry tar files found in ${HOSTMACHINE_REGISTRY_PATH}/${OS_VERSION}."
    ...
    select ACTION in "Install tar manually" "Download from network" "Skip (handle later)"; do
        case "$REPLY" in
            1)
                ...
                echo "  → ${HOSTMACHINE_REGISTRY_PATH}/${OS_VERSION}"
